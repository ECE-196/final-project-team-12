...
def updatePlot(i):
    ...
    if (27000 <= i[1] <= 36000) or (0 <= i[1] <= 9000):
        ...
        xMid = 63
        yMid = 59  # center the LiDAR to the car on the screen
...
cs = board.D26 #IO26
dc = board.D27 #IO47
reset = board.D14 #IO33
spi = busio.SPI(board.D13, MOSI=board.D12) #initialize bus
display_bus = displayio.FourWire(spi, command=dc, chip_select=cs, reset=reset, baudrate=230400) # 1000000
#SDA IO45
#SCK IO46
...
# Release any previous display and initialize pins
...
cs = board.D26  # Chip select
dc = board.D27  # Data/Command
reset = board.D14  # Reset pin
spi = busio.SPI(board.D13, MOSI=board.D12)  # SPI bus initialization
...
splash = displayio.Group()  # Create a group for managing display objects
display.root_group = splash  # Show the splash group on the display

# Create a bitmap and a palette for drawing
bitmap = displayio.Bitmap(128, 64, 2)  # 128x64 pixels, 2 colors (black and white)
palette = displayio.Palette(2)
palette[0] = 0x000000  # Black
palette[1] = 0xFFFFFF  # White

# Create a TileGrid to hold the bitmap and use the palette
tile_grid = displayio.TileGrid(bitmap, pixel_shader=palette)

# Create a new display group and add the tile grid to it
group = displayio.Group()
group.append(tile_grid)

# Set the root group to display the group on the screen
display.root_group = group

# Function to draw a single pixel on the display
def draw_pixel(x, y, color):
    if 0 <= x < 128 and 0 <= y < 64:  # Ensure the pixel is within bounds
        bitmap[x, y] = color
        time.sleep(0.1)
...
# Update loop for drawing pixels
while True:
    for x in range(128):
        draw_pixel(x, 0, 1)  # Draw a white pixel at (x, 0)
    time.sleep(0.1)
    for x in range(128):
        turnoff_pixel(x, 0)  # Turn off the pixel at (x, 0)
    time.sleep(0.1)
